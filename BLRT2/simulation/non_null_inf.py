...
from BLRT2.others.distribution import sur_nonnull_inf
...
def run(it, sizeX):
    print(it)
    seed = 1997 + 1107 * it
    result = [experiment(survival, scaleC=1.0, sizeX=sizeX, sizeB=1000, seed=seed) for survival in sur_nonnull_inf()]
    return result
...
def main(sizeX):
    ...
    with open('../data/non-null_inf_adj_{}'.format(sizeX), 'wb') as file:
        pickle.dump(R, file)
