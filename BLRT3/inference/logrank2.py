import numpy as np
from BLRT3.sampling.right_dirichlet2 import RightDirichlet
from lifelines.statistics import logrank_test

from BLRT3.others.distribution2 import sur_null
...
class RightLogRank:
    ...
    def dlrt(self, size, alpha=0.001, m=1000, tl=None, a=None, b=None):
        rd1 = RightDirichlet(self.X1, self.D1, size, alpha=alpha, tl=tl[0])
        rd2 = RightDirichlet(self.X2, self.D2, size, alpha=alpha, tl=tl[1])
...
def main(num, scaleC=1.0, sizeX=100, sizeB=1000):
    ...
    rlr.clrt()
    rlr.dlrt(size=sizeB, alpha=100, m=1000, tl=[sur_null()[num], sur_null()[num]], a=[4.0, 4.0], b=[4.0, 4.0])
    print(rlr.p)
